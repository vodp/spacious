...
static char local_id[] = "$Id: partibrains.c,v 1.123 2009/06/26 08:38:46 slevy Exp $";
...
specks_init( int argc, char *argv[] )
{
  ...
  st->boxes = NULL;
  st->boxlevels = 0;
  st->boxlinewidth = 0.75; //box width
...
void specks_read_mesh( struct stuff *st, FILE *f, int argc, char **argv, char *buf )
{
  ...
  struct mesh *m = NewN( struct mesh, 1 ); //create a temporary var to load mesh speck file
  ...
  for(i = 1; i+1 < argc; i += 2) {
	...
	timestep = atoi(argv[i+1]); //time step for each mesh
    } else if(!strncmp(argv[i], "-t", 2)) {
	havetx = sscanf(argv[i+1], "%d", &m->txno); //texture
    } else if(!strcmp(argv[i], "-static")) {
	timestep = -1; // static mesh
	i--;
    } else if(!strncmp(argv[i], "-c", 2)) {
	sscanf(argv[i+1], "%d", &m->cindex); // color for mesh
    } else if(!strncmp(argv[i], "-s", 2)) {
	m->style = getstyle(argv[i+1]); // mesh style
    } else if(!strncmp(argv[i], "-w", 2)) {
	m->linewidth = atof(argv[i+1]); // line width
    } else {
	break;
    }
  }
...
" kira {node|ring|size|scale|span|track}  starlab controls; try \"kira ?\"",
  ...
  } else if (!strcmp(argv[0],"grid"))
  {
	// Phong's code
	  if (argc < 3)
		  msg("Usage: grid <n_blocks_width> <n_blocks_height>");
	  else
	  {
		  int w = atoi(argv[1]);
		  int h = atoi(argv[2]);
  ...
  {
	  if (argc < 2)
		  msg("Usage: activate <block1 index> <block2 index> ...");
	  else
	  {
		  int* blocklist = NewA(int,argc-1);//(int*)malloc(sizeof(int)*(argc-1));
		  int nblocks = 0;
		  int i;
		  for (i = 1 ; i < argc ; ++i )
		  {
			  int idx = atoi(argv[i]);
			  if (idx == 0 && strcmp(argv[i],"0"))
				  continue;
			  blocklist[i-1] = idx;
			  nblocks++;
		  }

		  spac_activate_grid(blocklist,nblocks);
	  }
  }
  else if (!strcmp(argv[0],"imread"))
  {
	// Phong's code
	  if (!spac_imread (argv[1]))
			msg("Image file not found!");

  }else if (!strcmp(argv[0],"previmread"))
  {
	  if (argc < 2)
		  msg("Bad command format!");
	  else{
		  if (!spac_previmread (argv[1]))
			  msg("Image file not found!");
	  }

  }else if(!strcmp( argv[0], "bound" )) { // this is what I need
	register struct specklist *sl;
	register struct speck *sp;
