...
static char local_id[] = "$Id: partiview.cc,v 1.63 2010/09/07 19:51:04 slevy Exp $";
...
/*This command is binded with commandline widget, used to parse commands and call functions
 * from Partiview'engine (partibrains.c)*/
...
// Aymen's code
void initialize_choiceBoxes(int dim)
{
    ...
    {
    	sprintf(tmp, "%d",i);
    	ppui.choice1->add(tmp);
    	ppui.choice2->add(tmp);
    	ppui.choice3->add(tmp);
    	ppui.choice_color->add(tmp);
    }

    ppui.choice_color->add("none");

	ppui.choice1->value(0);
	ppui.choice2->value(1);
	ppui.choice3->value(2);
	ppui.choice_color->value(dim);
}

/*
void choice_changed(Fl_Widget* widget)
{
	if (widget->changed())
	{

		int dimchoice1 = ppui.choice1->value();
		int dimchoice2 = ppui.choice2->value();
		int dimchoice3 = ppui.choice3->value();
		spac_switch_dimensions(dimchoice1, dimchoice2, dimchoice3);
...
*/
  ...
  int dim = ppui.st->sl->specks->dim;
  initialize_choiceBoxes(dim);
/*
  ppui.choice1->callback(choice_changed);
  ppui.choice2->callback(choice_changed);
  ppui.choice3->callback(choice_changed);
  */
  //------------------
