...
int spac_setbound(int* idx, int n)
{
	...
	int patchid;
	specks_set_byvariable(st,"patchid",&patchid);
...
//#endif
	...
	{
		specks_commandfmt(&ppui.st, "only= patchid %d",idx[0]);
		for (i = 1; i < n ; ++i)
			specks_commandfmt(&ppui.st, "only+ patchid %d",idx[i]);
	}

	// activate subset of specks
	spac_activate_grid(idx,n);
	// spac_activate_grid(pointset,n);
...
int spac_previmread(const char* filename)
{
	// if another image was used before, release it
	if (ppui.previmg != 0)
	{
		//ppui.imgdisp->value(0);
		ppui.previmg->release();
		ppui.previmg = 0;
	}

	// load image from disk
	ppui.previmg = Fl_Shared_Image::get(filename);

	if (!ppui.previmg)
		return 0; // image not found
	else
		ppui.imgdisp->set_preview(ppui.previmg);
...
int spac_hitbox (Point cen, float xsize, float ysize, float zsize)
{
	...
	for(sl = st->sl, j = 0; sl != NULL; sl = sl->next)
	{
		...
		for(i = sl->nspecks; --i >= 0; sp = NextSpeck( sp, sl, 1 )) {
			...
			activated[j++] = sp->val[patchid]; // value index 1 means 'txnum'
		}
	}
...
// Aymen's code
void spac_switch_dimensions(int slot, int new_dim)
{
	register struct specklist *sl;
	register struct speck *sp;
	...
	if (new_dim < 0 || new_dim >= ppui.st->sl->specks->dim)
		return;
